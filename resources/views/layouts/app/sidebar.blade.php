        ...
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <flux:dropdown position="bottom" align="start">
                    ...
                    <flux:menu>
                        @foreach ($availableSites as $site)
                            <flux:menu.item :href="route('admin.site.dashboard', $site)" wire:navigate>{{ $site->name }} @if ($hasSite && $site->is($currentSite)) ✓ @endif</flux:menu.item>
                        @endforeach
                        <flux:menu.separator />
                        <flux:menu.item :href="route('site.create')" icon="plus" wire:navigate>Criar novo website</flux:menu.item>
                    </flux:menu>
                </flux:dropdown>
            </div>
            ...
            <flux:sidebar.nav>
                <flux:sidebar.group heading="Finder Hub" class="grid">
                    <flux:sidebar.item icon="squares-2x2" :href="route('home')" :current="request()->routeIs('home')" wire:navigate>Início / Projetos</flux:sidebar.item>
                    @if($hasSite)
                ...
                @if($hasSite)
                    ...
                    <flux:sidebar.group heading="Catálogo" class="grid">
                        <flux:sidebar.item icon="shopping-bag" :href="$siteRoute('products')" :current="request()->routeIs('admin.site.products')" wire:navigate>Produtos</flux:sidebar.item>
                        <flux:sidebar.item icon="tag" :href="$siteRoute('categories')" :current="request()->routeIs('admin.site.categories')" wire:navigate>Categorias</flux:sidebar.item>
                    </flux:sidebar.group>

                    <flux:sidebar.group heading="Vendas & Clientes" class="grid">
                        <flux:sidebar.item icon="clipboard-document-list" :href="$siteRoute('orders')" :current="request()->routeIs('admin.site.orders')" wire:navigate>Encomendas</flux:sidebar.item>
                ...
                @else
                    <div class="px-4 py-8 text-center">
                        <p class="text-xs font-bold uppercase tracking-widest text-zinc-500">Cria ou seleciona um website para começar.</p>
                        <flux:button :href="route('site.create')" size="sm" class="mt-4 !rounded-xl" variant="primary">Criar website</flux:button>
                    </div>
                @endif
            </flux:sidebar.nav>
            <flux:spacer />
