...
<body class="min-h-full bg-zinc-100 text-zinc-950 antialiased dark:bg-zinc-950 dark:text-white">
    ...
    @if(request()->routeIs('builder.edit'))
        ...
        <div x-data="{ open: false, category: 'sidebar' }" class="pointer-events-none fixed inset-0 z-[80] flex justify-center">
            ...
            <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="translate-y-2 opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="translate-y-0 opacity-100" x-transition:leave-end="translate-y-2 opacity-0" class="pointer-events-auto fixed left-1/2 top-[74px] w-[380px] max-w-[calc(100vw-24px)] -translate-x-1/2 rounded-3xl border border-zinc-200 bg-white p-4 shadow-2xl">
                ...
                <div class="mt-4 space-y-2 rounded-2xl border border-zinc-200 bg-zinc-50 p-1.5">
                    <button type="button" @click="category='sidebar'" class="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition" :class="category === 'sidebar' ? 'bg-zinc-950 text-white shadow-md' : 'text-zinc-700 hover:bg-white'">
                        ...
                        <span class="flex-1"><span class="block text-xs font-black">Sidebar</span><span class="block text-[10px] opacity-60">Esquerda, direita, em cima ou em baixo</span></span>
                    ...
                    <button type="button" @click="category='typography'" class="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition" :class="category === 'typography' ? 'bg-zinc-950 text-white shadow-md' : 'text-zinc-700 hover:bg-white'">
                        ...
                        <span class="flex-1"><span class="block text-xs font-black">Tipo de letra</span><span class="block text-[10px] opacity-60">Fonte, títulos, subtítulos e texto</span></span>
                        <span class="text-xs font-black">›</span>
                    </button>
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        @foreach($sidebarModels as $modelKey => [$modelName, $modelDescription, $modelIcon])
                            <button type="button" wire:click="$set('theme.layout', '{{ $modelKey }}'); $wire.save()" class="group flex min-h-[104px] flex-col items-start gap-2 rounded-2xl border p-3 text-left transition {{ data_get($builderTheme, 'layout', 'top') === $modelKey ? 'border-zinc-950 bg-zinc-950 text-white shadow-lg' : 'border-zinc-200 bg-white text-zinc-800 hover:border-zinc-400 hover:bg-zinc-50' }}">
                                <span class="flex h-9 w-9 items-center justify-center rounded-xl {{ data_get($builderTheme, 'layout', 'top') === $modelKey ? 'bg-white/10' : 'bg-zinc-100' }} text-base">{{ $modelIcon }}</span>
                                <span><span class="block text-xs font-black">{{ $modelName }}</span><span class="mt-0.5 block text-[9px] {{ data_get($builderTheme, 'layout', 'top') === $modelKey ? 'text-white/50' : 'text-zinc-400' }}">{{ $modelDescription }}</span></span>
                            </button>
                        @endforeach
                    </div>
                ...
                <div x-show="category === 'typography'" x-cloak x-transition class="mt-4">
                    ...
                    <div class="grid gap-2">
                        @foreach($fontModels as $fontKey => [$fontName, $fontLabel, $fontExample, $fontClass])
                            <button type="button" wire:click="$set('theme.font_family', '{{ $fontKey }}'); $wire.save()" class="group flex items-center gap-3 rounded-2xl border px-3 py-3 text-left transition {{ data_get($builderTheme, 'font_family', 'modern-sans') === $fontKey ? 'border-zinc-950 bg-zinc-950 text-white shadow-lg' : 'border-zinc-200 bg-white text-zinc-800 hover:border-zinc-400 hover:bg-zinc-50' }}">
                                <span class="flex h-12 w-14 items-center justify-center rounded-xl {{ data_get($builderTheme, 'font_family', 'modern-sans') === $fontKey ? 'bg-white/10' : 'bg-zinc-100' }} text-xl {{ $fontClass }}">{{ $fontExample }}</span>
                                <span class="min-w-0 flex-1"><span class="block text-xs font-black">{{ $fontLabel }}</span><span class="mt-0.5 block text-[10px] opacity-60">{{ $fontName }}</span></span>
                                @if(data_get($builderTheme, 'font_family', 'modern-sans') === $fontKey)<span class="text-xs font-black">✓</span>@endif
                            </button>
                        @endforeach
                    </div>
                    <div class="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-3"><p class="text-[9px] font-black uppercase tracking-[0.18em] text-zinc-400">Tamanhos</p><div class="mt-3 grid grid-cols-4 gap-2">
                        @foreach(['compact' => ['Pequeno', 'text-2xl', 'text-sm'], 'balanced' => ['Normal', 'text-3xl', 'text-base'], 'large' => ['Grande', 'text-4xl', 'text-lg'], 'display' => ['Display', 'text-5xl', 'text-xl']] as $sizeKey => [$sizeName, $headingClass, $bodyClass])
                            <button type="button" wire:click="$set('theme.typography_scale', '{{ $sizeKey }}'); $wire.save()" class="rounded-xl border p-2 text-left transition {{ data_get($builderTheme, 'typography_scale', 'balanced') === $sizeKey ? 'border-zinc-950 bg-zinc-950 text-white' : 'border-zinc-200 bg-white hover:border-zinc-400' }}"><span class="block text-[9px] font-black">{{ $sizeName }}</span><span class="mt-1 block truncate {{ $headingClass }} font-black">Título</span><span class="block truncate {{ $bodyClass }} opacity-60">Texto</span></button>
                        @endforeach
                    </div></div>
                    <div class="mt-3 rounded-2xl bg-zinc-950 p-4 text-white"><p class="text-[9px] font-black uppercase tracking-[0.18em] text-white/40">Pré-visualização</p><p class="mt-2 text-2xl font-black tracking-tight">Título principal</p><p class="mt-1 text-sm font-semibold text-white/60">Subtítulo da página</p><p class="mt-3 text-sm leading-6 text-white/55">Este texto mostra como a tipografia escolhida fica aplicada no website.</p></div>
                </div>
