...
<body class="min-h-full bg-zinc-100 text-zinc-950 antialiased dark:bg-zinc-950 dark:text-white">
    ...
    @if(request()->routeIs('builder.edit'))
        ...
        <div x-data="{ open: false, category: 'sidebar', selectedLayout: @js($builderLayout) }" class="pointer-events-none fixed inset-0 z-[80]">
            ...
            <div x-show="open" x-cloak x-transition @keydown.escape.window="open=false" class="pointer-events-auto fixed bottom-[148px] left-1/2 max-h-[calc(100vh-170px)] w-[410px] max-w-[calc(100vw-24px)] -translate-x-1/2 overflow-y-auto overscroll-contain rounded-3xl border border-zinc-200 bg-white p-4 pb-5 shadow-2xl">
                <div class="flex items-start justify-between gap-3"><div><p class="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Design do website</p><h2 class="mt-1 text-lg font-black tracking-tight">Modelos</h2><p class="mt-1 text-[11px] leading-5 text-zinc-400">Escolhe a estrutura, fonte e tamanhos. O modal permanece aberto enquanto personalizas.</p></div><button type="button" @click="open=false" class="h-8 w-8 shrink-0 rounded-xl bg-zinc-100 text-zinc-500">×</button></div>
                ...
                <div x-show="category === 'sidebar'" x-transition class="mt-4">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        @foreach($sidebarModels as $modelKey => [$modelName, $modelDescription, $modelIcon])
                            <button type="button" @click="selectedLayout = '{{ $modelKey }}'" wire:click="$set('theme.layout', '{{ $modelKey }}'); save()" class="group flex min-h-[104px] flex-col items-start gap-2 rounded-2xl border p-3 text-left transition" :class="selectedLayout === '{{ $modelKey }}' ? 'border-zinc-950 bg-zinc-950 text-white shadow-lg' : 'border-zinc-200 bg-white text-zinc-800 hover:border-zinc-400 hover:bg-zinc-50'"><span class="flex h-9 w-9 items-center justify-center rounded-xl" :class="selectedLayout === '{{ $modelKey }}' ? 'bg-white/10' : 'bg-zinc-100'">{{ $modelIcon }}</span><span><span class="block text-xs font-black">{{ $modelName }}</span><span class="mt-0.5 block text-[9px]" :class="selectedLayout === '{{ $modelKey }}' ? 'text-white/50' : 'text-zinc-400'">{{ $modelDescription }}</span></span></button>
                        @endforeach
                    </div>
