        ...
        foreach ($nodes as $node) {
            ...
            switch ($type) {
                case 'heading':
                    ...
                    echo '<'.$level.' wire:click.stop="selectElement(\''.e($nodeId).'\')" contenteditable="true" spellcheck="true" x-on:focus="$wire.selectElement(\''.e($nodeId).'\')" x-on:blur="window.dispatchEvent(new CustomEvent(\'builder-autosave\')); $wire.updateSelectedElement(\'text\', $el.innerText).then(() => $wire.save()).then(() => window.dispatchEvent(new CustomEvent(\'builder-autosave-finished\'))).catch(() => window.dispatchEvent(new CustomEvent(\'builder-autosave-error\')))" x-on:keydown.enter.prevent="$el.blur()" class="font-bold tracking-tight '.$size.' cursor-text outline-none transition hover:ring-2 hover:ring-blue-200 focus:ring-2 focus:ring-blue-400 rounded-lg">'.e((string) ($content['text'] ?? '')).'</'.$level.'>';
                    break;
                case 'text':
                    echo '<p wire:click.stop="selectElement(\''.e($nodeId).'\')" contenteditable="true" spellcheck="true" x-on:focus="$wire.selectElement(\''.e($nodeId).'\')" x-on:blur="window.dispatchEvent(new CustomEvent(\'builder-autosave\')); $wire.updateSelectedElement(\'text\', $el.innerText).then(() => $wire.save()).then(() => window.dispatchEvent(new CustomEvent(\'builder-autosave-finished\'))).catch(() => window.dispatchEvent(new CustomEvent(\'builder-autosave-error\')))" class="whitespace-pre-line leading-8 text-zinc-600 dark:text-zinc-300 cursor-text outline-none transition hover:ring-2 hover:ring-blue-200 focus:ring-2 focus:ring-blue-400 rounded-lg">'.e((string) ($content['text'] ?? '')).'</p>';
                    break;
                case 'image':
                ...
                case 'quote':
                    echo '<blockquote class="border-l-4 border-[var(--finder-primary)] pl-5"><p wire:click.stop="selectElement(\''.e($nodeId).'\')" contenteditable="true" spellcheck="true" x-on:focus="$wire.selectElement(\''.e($nodeId).'\')" x-on:blur="window.dispatchEvent(new CustomEvent(\'builder-autosave\')); $wire.updateSelectedElement(\'text\', $el.innerText).then(() => $wire.save()).then(() => window.dispatchEvent(new CustomEvent(\'builder-autosave-finished\'))).catch(() => window.dispatchEvent(new CustomEvent(\'builder-autosave-error\')))" class="text-xl leading-8 cursor-text outline-none hover:ring-2 hover:ring-blue-200 focus:ring-2 focus:ring-blue-400 rounded-lg">“'.e((string)($content['text'] ?? '')).'”</p>'.(!empty($content['author']) ? '<footer class="mt-3 text-sm text-zinc-500">'.e((string)$content['author']).'</footer>' : '').'</blockquote>';
                    break;
                case 'video':
