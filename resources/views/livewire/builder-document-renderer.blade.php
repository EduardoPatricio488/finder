@php
    ...
    $renderNodes = static function (array $nodes) use (&$renderNodes, $device): void {
        foreach ($nodes as $node) {
            ...
            echo '<div class="builder-element '.$alignClass.'"'.$styleAttribute.'>';
            switch ($type) {
                case 'heading':
                    ...
                    echo '<'.$level.' wire:click.stop="selectElement(\''.e($nodeId).'\')" contenteditable="true" spellcheck="true" x-on:focus="$wire.selectElement(\''.e($nodeId).'\')" x-on:blur="$wire.updateSelectedElement(\'text\', $el.innerText)" x-on:keydown.enter.prevent="$el.blur()" class="font-bold tracking-tight '.$size.' cursor-text outline-none transition hover:ring-2 hover:ring-blue-200 focus:ring-2 focus:ring-blue-400 rounded-lg">'.e((string) ($content['text'] ?? '')).'</'.$level.'>';
                    break;
                case 'text':
                    echo '<p wire:click.stop="selectElement(\''.e($nodeId).'\')" contenteditable="true" spellcheck="true" x-on:focus="$wire.selectElement(\''.e($nodeId).'\')" x-on:blur="$wire.updateSelectedElement(\'text\', $el.innerText)" class="whitespace-pre-line leading-8 text-zinc-600 dark:text-zinc-300 cursor-text outline-none transition hover:ring-2 hover:ring-blue-200 focus:ring-2 focus:ring-blue-400 rounded-lg">'.e((string) ($content['text'] ?? '')).'</p>';
                    break;
                case 'image':
                    ...
                    break;
                case 'button':
                    echo '<a href="#" wire:click.stop.prevent="selectElement(\''.e($nodeId).'\')" contenteditable="true" spellcheck="true" x-on:focus="$wire.selectElement(\''.e($nodeId).'\')" x-on:blur="$wire.updateSelectedElement(\'label\', $el.innerText)" x-on:keydown.enter.prevent="$el.blur()" class="inline-flex rounded-xl px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:-translate-y-0.5 cursor-text outline-none focus:ring-2 focus:ring-blue-400" style="background:var(--finder-primary)">'.e((string) ($content['label'] ?? 'Saber mais')).'</a>';
                    break;
                ...
                case 'quote':
                    echo '<blockquote class="border-l-4 border-[var(--finder-primary)] pl-5"><p wire:click.stop="selectElement(\''.e($nodeId).'\')" contenteditable="true" spellcheck="true" x-on:focus="$wire.selectElement(\''.e($nodeId).'\')" x-on:blur="$wire.updateSelectedElement(\'text\', $el.innerText)" class="text-xl leading-8 cursor-text outline-none hover:ring-2 hover:ring-blue-200 focus:ring-2 focus:ring-blue-400 rounded-lg">“'.e((string)($content['text'] ?? '')).'”</p>'.(!empty($content['author']) ? '<footer class="mt-3 text-sm text-zinc-500">'.e((string)$content['author']).'</footer>' : '').'</blockquote>';
                    break;
                case 'video':
