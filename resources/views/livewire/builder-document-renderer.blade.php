@php
    ...
    $renderNodes = static function (array $nodes) use (&$renderNodes, $device, $selectedElementId, $sectionIndex): void {
        foreach ($nodes as $node) {
            ...
            echo '<button type="button" wire:click.stop="removeElement(\''.e($nodeId).'\', '.$sectionIndex.')" title="Remover elemento" class="absolute -right-2 -top-2 z-10 hidden h-5 w-5 items-center justify-center rounded-full bg-red-600 text-[10px] font-bold text-white shadow group-hover/el:flex">×</button>';
            switch ($type) {
                case 'heading':
