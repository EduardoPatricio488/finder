@php
    ...
    $renderNodes = static function (array $nodes) use (&$renderNodes, $device): void {
        foreach ($nodes as $node) {
            ...
            if ($type === 'container') {
                echo '<div class="mx-auto w-full max-w-7xl '.$spacing.' '.$alignClass.'"'.$styleAttribute.'>';
            ...
            switch ($type) {
                ...
                case 'image':
                    ...
                    if ($url !== '' && preg_match('/^(https?:\/\/|\/|storage\/)/i', $url)) {
                        ...
                        if (! empty($content['caption'])) {
                            echo '<figcaption class="mt-3 text-center text-sm text-zinc-500">'.e((string) $content['caption']).'</figcaption>';
                        }
                        echo '</figure>';
                    }
                    break;
                ...
                case 'divider':
                    ...
                    break;
                case 'spacer':
                    $height = match ($settings['height'] ?? 'md') {
                        'sm' => 'h-4',
                        'lg' => 'h-16',
                        'xl' => 'h-28',
                        default => 'h-8',
                ...
                case 'video':
                    ...
                    if ($url !== '' && preg_match('/^https:\/\//i', $url)) {
                        echo '<div class="aspect-video overflow-hidden rounded-3xl bg-black"><iframe src="'.e($url).'" title="'.e((string) ($content['title'] ?? 'Vídeo')).'" class="h-full w-full" loading="lazy" allowfullscreen></iframe></div>';
                    }
                    break;
                case 'gallery':
                    ...
                    foreach ($items as $item) {
                        if (! empty($item['url']) && preg_match('/^(https?:\/\/|\/|storage\/)/i', (string) $item['url'])) {
                            echo '<img src="'.e((string) $item['url']).'" alt="'.e((string) ($item['alt'] ?? '')).'" loading="lazy" class="aspect-[4/3] w-full rounded-2xl object-cover">';
                        }
                    }
                    echo '</div>';
                    break;
                case 'faq':
                    ...
                    foreach (array_values(array_filter($content['items'] ?? [], 'is_array')) as $item) {
                        echo '<details class="py-4"><summary class="cursor-pointer font-semibold">'.e((string) ($item['question'] ?? $item['title'] ?? '')).'</summary><p class="mt-2 leading-7 text-zinc-600 dark:text-zinc-300">'.e((string) ($item['answer'] ?? $item['description'] ?? '')).'</p></details>';
                    }
                    echo '</div></div>';
                    break;
                case 'social_links':
