        ...
        <main>
            <section class="relative isolate overflow-hidden px-5 pb-20 pt-16 sm:px-8 sm:pb-28 sm:pt-24">
                ...
                <div class="mx-auto flex max-w-3xl flex-col items-center">
                    ...
                    <h1 class="tracking-[-0.045em]">
                        {{ $site->name }}
                    </h1>
                    <p class="mt-7 max-w-2xl text-zinc-600">
                        {{ $description }}
                    </p>
                    @if($audience !== '')
                        <p class="mt-3 max-w-2xl text-sm font-medium text-zinc-500">
                            Trabalho principalmente com <span class="font-semibold text-zinc-800">{{ $audience }}</span>.
                        </p>
                    @endif
                    <div class="mt-9 flex flex-wrap items-center justify-center gap-3">
                        <a href="{{ route('site.public', [$site, 'pageSlug' => 'about', 'preview' => $preview ? 1 : null]) }}" class="inline-flex items-center gap-2 bg-zinc-950 px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-zinc-900/10 transition hover:-translate-y-0.5">
                            Conhecer-me <span aria-hidden="true">→</span>
                        </a>
                        <a href="{{ route('site.public', [$site, 'pageSlug' => 'contact', 'preview' => $preview ? 1 : null]) }}" class="inline-flex border border-black/10 bg-white px-6 py-3.5 text-sm font-semibold text-zinc-800 transition hover:-translate-y-0.5 hover:border-black/20">
                            {{ $goalLabel }}
                        </a>
                    </div>
                        ...
                        <div class="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/80 p-3 shadow-2xl shadow-zinc-900/10 backdrop-blur">
                            <div class="rounded-[1.5rem] bg-zinc-950 p-7 text-white sm:p-9">
                                ...
                                <div class="mt-12 flex items-center justify-between border-t border-white/10 pt-5 text-xs text-white/45">
                                    <span>{{ $categoryLabel !== '' ? $categoryLabel : 'Personal' }}</span>
                                    <span>PT-PT</span>
                                </div>
                            </div>
                        </div>
            ...
            {{-- Barra de destaques rápidos, gerada a partir do que já preencheste na criação do site --}}
            <section class="border-y border-black/5 bg-white px-5 py-10 sm:px-8">
                <div class="mx-auto grid max-w-6xl gap-6 sm:grid-cols-3">
                    <div class="flex items-start gap-3">
                        <span class="mt-1 h-2 w-2 shrink-0 rounded-full" style="background: var(--finder-primary)"></span>
                        <div><p class="text-xs font-bold uppercase tracking-wider text-zinc-400">Área</p><p class="mt-1 font-semibold">{{ $categoryLabel !== '' ? $categoryLabel : 'Trabalho independente' }}</p></div>
                    </div>
                    <div class="flex items-start gap-3">
                        <span class="mt-1 h-2 w-2 shrink-0 rounded-full" style="background: var(--finder-primary)"></span>
                        <div><p class="text-xs font-bold uppercase tracking-wider text-zinc-400">Para quem</p><p class="mt-1 font-semibold">{{ $audience !== '' ? $audience : 'Todos os que procuram um serviço de confiança' }}</p></div>
                    </div>
                    <div class="flex items-start gap-3">
                        <span class="mt-1 h-2 w-2 shrink-0 rounded-full" style="background: var(--finder-primary)"></span>
                        <div><p class="text-xs font-bold uppercase tracking-wider text-zinc-400">Disponibilidade</p><p class="mt-1 font-semibold">{{ $availabilityNote }}</p></div>
                    </div>
                </div>
            </section>
            ...
            <section class="bg-white px-5 py-16 sm:px-8 sm:py-20">
                <div class="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.75fr_1.25fr] lg:gap-20">
                    ...
                    <div>
                        ...
                        @if($additionalInfo !== '')
                            <p class="mt-5 rounded-2xl border-l-4 bg-zinc-50 py-4 pl-5 pr-4 text-base leading-7 text-zinc-600" style="border-color: var(--finder-primary)">{{ $additionalInfo }}</p>
                        @endif
            ...
            {{-- Como posso ajudar — construído a partir da informação real que já deste sobre o teu trabalho --}}
            <section class="px-5 py-16 sm:px-8 sm:py-20">
                <div class="mx-auto max-w-6xl">
                    <p class="text-xs font-bold uppercase tracking-[0.18em]" style="color: var(--finder-primary)">Como posso ajudar</p>
                    <h2 class="mt-3 max-w-2xl text-3xl font-semibold tracking-tight sm:text-4xl">O que podes esperar de trabalhar comigo.</h2>
                    <div class="mt-10 grid gap-5 sm:grid-cols-3">
                        <div class="rounded-3xl border border-black/5 bg-white p-7 shadow-sm">
                            <span class="flex h-10 w-10 items-center justify-center rounded-xl text-sm font-bold text-white" style="background: var(--finder-primary)">01</span>
                            <p class="mt-5 font-semibold">O que faço</p>
                            <p class="mt-2 text-sm leading-6 text-zinc-500">{{ \Illuminate\Support\Str::limit($description, 160) }}</p>
                        </div>
                        <div class="rounded-3xl border border-black/5 bg-white p-7 shadow-sm">
                            <span class="flex h-10 w-10 items-center justify-center rounded-xl text-sm font-bold text-white" style="background: var(--finder-primary)">02</span>
                            <p class="mt-5 font-semibold">Para quem trabalho</p>
                            <p class="mt-2 text-sm leading-6 text-zinc-500">{{ $audience !== '' ? "Trabalho principalmente com {$audience}, adaptando sempre a abordagem a cada caso." : 'Trabalho com qualquer pessoa ou empresa que procure um serviço próximo e de confiança.' }}</p>
                        </div>
                        <div class="rounded-3xl border border-black/5 bg-white p-7 shadow-sm">
                            <span class="flex h-10 w-10 items-center justify-center rounded-xl text-sm font-bold text-white" style="background: var(--finder-primary)">03</span>
                            <p class="mt-5 font-semibold">Como trabalho</p>
                            <p class="mt-2 text-sm leading-6 text-zinc-500">{{ $additionalInfo !== '' ? \Illuminate\Support\Str::limit($additionalInfo, 160) : 'Comunicação clara, prazos cumpridos e atenção ao detalhe em cada etapa do processo.' }}</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="bg-white px-5 py-16 sm:px-8 sm:py-20">
                <div class="mx-auto max-w-4xl text-center">
                    <p class="text-xs font-bold uppercase tracking-[0.18em]" style="color: var(--finder-primary)">O que dizem</p>
                    <blockquote class="mt-5 text-2xl font-medium leading-tight tracking-tight text-zinc-800 sm:text-3xl">"Simples, directo e sempre disponível para esclarecer dúvidas. Recomendo sem hesitar."</blockquote>
                    <p class="mt-5 text-sm font-semibold text-zinc-500">Cliente satisfeito</p>
                </div>
            </section>
                ...
                <div class="mx-auto max-w-6xl rounded-[2rem] bg-zinc-950 px-7 py-12 text-white sm:px-12 sm:py-16">
                    <div class="flex flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
                        <div class="max-w-2xl">
                            ...
                            <p class="mt-4 text-white/60">{{ $availabilityNote }}</p>
                        </div>
                        <a href="{{ route('site.public', [$site, 'pageSlug' => 'contact', 'preview' => $preview ? 1 : null]) }}" class="inline-flex shrink-0 rounded-full px-6 py-3.5 text-sm font-semibold text-white transition hover:-translate-y-0.5" style="background: var(--finder-primary)">{{ $goalLabel }} →</a>
                    </div>
                </div>
            </section>
        </main>
        ...
        <main>
            <section class="px-5 pb-16 pt-20 sm:px-8 sm:pb-24 sm:pt-28">
                <div class="mx-auto max-w-5xl">
                    <p class="text-xs font-bold uppercase tracking-[0.18em]" style="color: var(--finder-primary)">{{ $categoryLabel !== '' ? $categoryLabel : 'Sobre mim' }}</p>
            ...
            <section class="bg-white px-5 py-16 sm:px-8 sm:py-24">
                <div class="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.65fr_1.35fr] lg:gap-24">
                    ...
                    <div class="space-y-6 text-lg leading-9 text-zinc-600">
                        <p>{{ $description }}</p>
                        <p>{{ $additionalInfo !== '' ? $additionalInfo : 'Este espaço foi criado para apresentar o essencial de forma clara e profissional. À medida que o conteúdo for completado, pode receber mais detalhes sobre experiência, interesses, trabalho ou projectos.' }}</p>
                    </div>
                </div>
            </section>
            ...
            @if($audience !== '')
                <section class="px-5 py-16 sm:px-8 sm:py-20">
                    <div class="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[.65fr_1.35fr] lg:gap-24">
                        <div><span class="text-sm font-semibold text-zinc-400">02</span><h2 class="mt-2 text-2xl font-semibold">Para quem trabalho</h2></div>
                        <div class="text-lg leading-9 text-zinc-600"><p>Trabalho sobretudo com <span class="font-semibold text-zinc-900">{{ $audience }}</span>, adaptando sempre a forma de trabalhar às necessidades específicas de cada pessoa ou projecto.</p></div>
                    </div>
                </section>
            @endif

            <section class="bg-white px-5 py-16 sm:px-8 sm:py-24">
                <div class="mx-auto max-w-6xl rounded-[2rem] border border-black/5 bg-[#f8f8f6] p-8 shadow-sm sm:p-12">
                    <div class="grid gap-8 sm:grid-cols-4">
                        ...
                        <div><p class="text-sm text-zinc-400">04</p><p class="mt-2 font-semibold">{{ $style !== '' ? $style : 'Consistência' }}</p><p class="mt-2 text-sm leading-6 text-zinc-500">Um estilo próprio, mantido em cada detalhe do trabalho.</p></div>
                    </div>
                </div>
            </section>

            <section class="px-5 py-16 sm:px-8 sm:py-20">
                <div class="mx-auto max-w-6xl rounded-[2rem] bg-zinc-950 px-7 py-12 text-white text-center sm:px-12 sm:py-16">
                    <h2 class="text-3xl font-semibold tracking-tight sm:text-4xl">{{ $availabilityNote }}</h2>
                    <a href="{{ route('site.public', [$site, 'pageSlug' => 'contact', 'preview' => $preview ? 1 : null]) }}" class="mt-7 inline-flex rounded-full px-6 py-3.5 text-sm font-semibold text-white transition hover:-translate-y-0.5" style="background: var(--finder-primary)">{{ $goalLabel }} →</a>
                </div>
            </section>
        </main>
    @else
                ...
                <div class="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[.8fr_1.2fr] lg:gap-24">
                    <div>
                        ...
                        <div class="mt-10 space-y-4">
                            <div class="flex items-start gap-3 rounded-2xl border border-black/5 bg-white p-4">
                                <span class="mt-0.5 h-2 w-2 shrink-0 rounded-full" style="background: var(--finder-primary)"></span>
                                <div><p class="text-sm font-semibold">Disponibilidade</p><p class="mt-0.5 text-sm text-zinc-500">{{ $availabilityNote }}</p></div>
                            </div>
                            @if($audience !== '')
                                <div class="flex items-start gap-3 rounded-2xl border border-black/5 bg-white p-4">
                                    <span class="mt-0.5 h-2 w-2 shrink-0 rounded-full" style="background: var(--finder-primary)"></span>
                                    <div><p class="text-sm font-semibold">Trabalho com</p><p class="mt-0.5 text-sm text-zinc-500">{{ $audience }}</p></div>
                                </div>
                            @endif
                        </div>
                    </div>
                    <div class="rounded-[2rem] border border-black/5 bg-white p-6 shadow-xl shadow-zinc-900/5 sm:p-9">
                        <form wire:submit="submitContact" class="grid gap-5">
                            ...
                            <button type="submit" class="inline-flex w-full items-center justify-center rounded-2xl px-5 py-3.5 text-sm font-semibold text-white transition hover:-translate-y-0.5" style="background: var(--finder-primary)">{{ $goalLabel }}</button>
                        </form>
                    </div>
                </div>
            </section>
