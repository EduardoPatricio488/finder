                        ...
                        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            @foreach($templates as $key => [$name, $description, $preview, $icon])
                                <button type="button" wire:click="apply('{{ $key }}')" wire:loading.attr="disabled" class="group overflow-hidden rounded-2xl border border-zinc-200 bg-white text-left transition hover:-translate-y-1 hover:border-indigo-400 hover:shadow-xl dark:border-zinc-700 dark:bg-zinc-950">
                                    ...
                                    <div class="p-4"><div class="flex items-center justify-between gap-2"><h3 class="font-black">{{ $name }}</h3>@if(data_get($site->theme, 'template') === $key)<span class="rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-black uppercase text-emerald-700">Atual</span>@endif</div><p class="mt-1 text-xs leading-5 text-zinc-500">{{ $description }}</p><span class="mt-4 inline-flex rounded-xl border border-zinc-200 px-3 py-2 text-[10px] font-black group-hover:border-indigo-200 group-hover:bg-indigo-50 group-hover:text-indigo-700 dark:border-zinc-700 dark:group-hover:bg-indigo-950/40">Aplicar modelo →</span></div>
                                </button>
                            @endforeach
                        </div>
