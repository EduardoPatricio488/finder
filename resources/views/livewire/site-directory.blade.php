<div class="min-h-screen bg-[#fcfaf7] text-stone-900 font-sans selection:bg-amber-200">
    ...
    <div class="flex items-center gap-6">
        {{-- Botão Entrar: Estilo Minimalista --}}
        <a href="{{ route('login') }}" wire:navigate class="text-[10px] font-black uppercase tracking-[0.2em] text-stone-500 hover:text-stone-900 transition-colors">
            Entrar
        </a>

        {{-- Botão Criar Conta: Estilo Call to Action --}}
        <flux:button
            href="{{ route('register') }}"
            variant="primary"
            class="!rounded-xl !bg-stone-950 !px-6 !py-2.5 !text-[10px] !font-black !uppercase !tracking-[0.2em] !text-white shadow-xl shadow-stone-950/20 hover:!bg-stone-800 transition-all transform hover:-translate-y-0.5 active:scale-95"
        >
            Criar Conta
        </flux:button>
    </div>
@endauth
    ...
    <main class="mx-auto max-w-7xl px-6 py-12 lg:px-12 lg:py-20 text-left">
        ...
        <header class="mb-24 flex flex-col items-start justify-between gap-10 md:flex-row md:items-end">
            <div class="max-w-2xl text-left">
                ...
                <p class="mt-8 text-xl font-medium leading-relaxed text-stone-500">Gira os teus projetos ou descobre o que a comunidade está a construir.</p>
            </div>

            @auth
                @if ($canCreateSites)
                    <flux:button wire:click="create" variant="primary" class="!rounded-2xl !bg-stone-950 !px-10 !py-5 shadow-2xl hover:scale-105 transition-all font-black uppercase tracking-widest text-xs">
                        Lançar Novo Site 🚀
                    </flux:button>
                @else
                    <flux:button href="{{ route('saas.upgrade') }}" variant="primary" class="!rounded-2xl !bg-amber-400 !text-stone-950 !px-10 !py-5 shadow-2xl hover:scale-105 transition-all font-black uppercase tracking-widest text-xs border-none">
                        Obter Premium para Criar 💎
                    </flux:button>
                @endif
            @endauth
        </header>
        ...
        <article wire:key="site-{{ $site->id }}" class="group flex flex-col overflow-hidden rounded-[2.5rem] border border-stone-200 bg-white transition-all hover:shadow-2xl hover:-translate-y-1">
            ...
            <div @class([
                ...
                <div class="relative z-10 flex items-center justify-between">
                    <span class="rounded-full bg-white/20 px-3 py-1 text-[9px] font-black uppercase tracking-widest backdrop-blur-md">
            ...
            {{-- 2. CORPO COM INFORMAÇÃO DETALHADA --}}
            <div class="flex flex-1 flex-col p-8 text-left">
                <p class="text-sm font-medium leading-relaxed text-stone-500 line-clamp-2 h-10 mb-8 italic">
                    "{{ $site->tagline ?? 'Um projeto incrível em desenvolvimento.' }}"
                </p>
                ...
                <div class="grid grid-cols-2 gap-4 mb-8">
                    <div class="rounded-2xl bg-stone-50 p-4 border border-stone-100 transition-colors group-hover:bg-white group-hover:border-amber-200">
                        <div class="flex items-center gap-2 mb-1">
                            <flux:icon name="shopping-bag" variant="micro" class="text-amber-500 size-4" />
                            <span class="text-[9px] font-black uppercase text-stone-400 tracking-widest">Catálogo</span>
                        </div>
                    ...
                    <div class="rounded-2xl bg-stone-50 p-4 border border-stone-100 transition-colors group-hover:bg-white group-hover:border-emerald-200">
                        <div class="flex items-center gap-2 mb-1">
                            <flux:icon name="chart-bar" variant="micro" class="text-emerald-500 size-4" />
                            <span class="text-[9px] font-black uppercase text-stone-400 tracking-widest">Vendas</span>
                        </div>
                ...
                {{-- Rodapé do Card com Info do Dono --}}
                <div class="mt-auto flex items-center justify-between pt-6 border-t border-stone-100">
                    <div class="flex items-center gap-3">
                        <div class="size-8 rounded-full bg-stone-100 border border-stone-200 flex items-center justify-center text-[10px] font-black text-stone-500 uppercase shadow-inner">
                            {{ substr($site->owner->name ?? 'A', 0, 2) }}
                        </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @endif
                        <flux:button href="{{ $site->destinationUrl() }}" variant="primary"
                            @class([
                                '!rounded-xl !py-2.5 !px-5 !font-black !uppercase !text-[10px] !tracking-widest !border-none shadow-lg transition-all hover:scale-105',
                                'bg-amber-400 text-stone-950 shadow-amber-500/20' => $site->accent === 'amber',
                                'bg-stone-950 text-white shadow-stone-950/20' => $site->accent === 'stone',
                                'bg-emerald-500 text-white shadow-emerald-500/20' => $site->accent === 'emerald',
                                'bg-sky-500 text-white shadow-sky-500/20' => $site->accent === 'sky',
                            ])>
                            Visitar
                        </flux:button>
                    </div>
    ...
    @endforeach
        ...
        {{-- 4. SECÇÃO DE PLANOS (ID PARA O ANCHOR) --}}
        <div id="planos" class="mt-32 pt-10">
             {{-- Aqui podes manter a tua secção de preços anterior --}}
        </div>
    </main>
</div>
