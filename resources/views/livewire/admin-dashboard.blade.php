<div class="mx-auto max-w-7xl space-y-8 px-6 py-10 lg:px-12">
    ...
    <div class="grid gap-8 lg:grid-cols-2">
        <section class="rounded-[2rem] border border-stone-200 bg-white p-7 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"><div class="flex items-center justify-between"><div><h2 class="text-lg font-black text-stone-950 dark:text-white">Utilizadores recentes</h2><p class="mt-1 text-sm text-stone-500">Últimos membros associados ao website.</p></div><flux:button href="{{ route('users') }}" variant="ghost" size="sm">Ver todos</flux:button></div><div class="mt-6 space-y-3">@forelse($recentUsers as $user)<div class="flex items-center justify-between rounded-2xl bg-stone-50 px-4 py-3 dark:bg-zinc-800/60"><div><p class="text-sm font-semibold text-stone-900 dark:text-white">{{ $user->name }}</p><p class="text-xs text-stone-500">{{ $user->email }}</p></div></div>@empty<p class="rounded-2xl bg-stone-50 p-5 text-sm text-stone-500 dark:bg-zinc-800/60">Ainda não existem utilizadores adicionais.</p>@endforelse</div></section>
