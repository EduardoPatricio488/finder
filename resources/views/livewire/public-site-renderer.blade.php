...
>
    ...
    <header class="sticky top-0 z-30 border-b border-black/5 bg-white/85 backdrop-blur dark:bg-zinc-950/85">
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-4">
            ...
            @endif
            <a href="{{ route('site.public', $site) }}" class="rounded-xl px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2" style="background: var(--finder-primary)">Início</a>
        </div>
    </header>
            ...
            @switch($section->type)
                @case('hero')
                    <section class="px-5 py-24 text-center sm:py-32">
                        <div class="mx-auto max-w-4xl">
                            ...
                            <p class="mx-auto mt-6 max-w-2xl text-lg leading-8 text-zinc-600 dark:text-zinc-300">{{ $content['subtitle'] ?? $site->tagline }}</p>
                            @if(!empty($content['button_label']))
                                <a href="{{ $content['button_url'] ?? '#' }}" class="mt-8 inline-flex rounded-xl px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--finder-primary)] focus-visible:ring-offset-2" style="background: var(--finder-primary)">{{ $content['button_label'] }}</a>
                            @endif
                        </div>
                    </section>
                    @break
                ...
                @case('text')
                    <section class="mx-auto max-w-3xl px-5 py-16">
                        @if(!empty($content['title']))<h2 class="text-3xl font-semibold tracking-tight">{{ $content['title'] }}</h2>@endif
                        <p class="mt-4 whitespace-pre-line leading-8 text-zinc-600 dark:text-zinc-300">{{ $content['body'] ?? $content['description'] ?? '' }}</p>
                    </section>
                    @break

                @case('image')
                    <section class="mx-auto max-w-6xl px-5 py-16">
                        @if(!empty($content['url']))<figure><img src="{{ $content['url'] }}" alt="{{ $content['alt'] ?? '' }}" loading="lazy" class="mx-auto max-h-[700px] w-full rounded-3xl object-cover shadow-sm">@if(!empty($content['caption']))<figcaption class="mt-3 text-center text-sm text-zinc-500">{{ $content['caption'] }}</figcaption>@endif</figure>@endif
                    </section>
                    @break
                ...
                @case('contact_form')
                    <section class="mx-auto max-w-2xl px-5 py-16"><div class="rounded-3xl border border-zinc-200 p-7 shadow-sm dark:border-zinc-800"><h2 class="text-2xl font-semibold">{{ $content['title'] ?? 'Contacta-nos' }}</h2><p class="mt-2 text-sm text-zinc-500">{{ $content['description'] ?? 'Envia-nos uma mensagem.' }}</p><form wire:submit="submitContact" class="mt-6 grid gap-4"><div><label for="contact-name" class="sr-only">Nome</label><input id="contact-name" wire:model="contactName" type="text" autocomplete="name" placeholder="Nome" class="w-full rounded-xl border border-zinc-200 px-4 py-3 outline-none focus:border-[var(--finder-primary)] focus:ring-2 focus:ring-[var(--finder-primary)]/20 dark:border-zinc-700 dark:bg-zinc-900"></div><div><label for="contact-email" class="sr-only">Email</label><input id="contact-email" wire:model="contactEmail" type="email" autocomplete="email" placeholder="Email" class="w-full rounded-xl border border-zinc-200 px-4 py-3 outline-none focus:border-[var(--finder-primary)] focus:ring-2 focus:ring-[var(--finder-primary)]/20 dark:border-zinc-700 dark:bg-zinc-900"></div><div><label for="contact-message" class="sr-only">Mensagem</label><textarea id="contact-message" wire:model="contactMessage" rows="5" placeholder="Mensagem" class="w-full rounded-xl border border-zinc-200 px-4 py-3 outline-none focus:border-[var(--finder-primary)] focus:ring-2 focus:ring-[var(--finder-primary)]/20 dark:border-zinc-700 dark:bg-zinc-900"></textarea></div>@error('contactName')<p class="text-sm text-red-600">{{ $message }}</p>@enderror @error('contactEmail')<p class="text-sm text-red-600">{{ $message }}</p>@enderror @error('contactMessage')<p class="text-sm text-red-600">{{ $message }}</p>@enderror<button type="submit" wire:loading.attr="disabled" class="rounded-xl px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-60" style="background:var(--finder-primary)">Enviar mensagem</button></form></div></section>
                    @break

                @case('newsletter')
                    <section class="mx-auto max-w-3xl px-5 py-16"><div class="rounded-3xl p-8 text-center" style="background:color-mix(in srgb,var(--finder-primary) 10%,transparent)"><h2 class="text-2xl font-semibold">{{ $content['title'] ?? 'Recebe novidades' }}</h2><form wire:submit="subscribeNewsletter" class="mx-auto mt-6 flex max-w-lg gap-2"><label for="newsletter-email" class="sr-only">Email</label><input id="newsletter-email" wire:model="newsletterEmail" type="email" autocomplete="email" placeholder="O teu email" class="min-w-0 flex-1 rounded-xl border border-zinc-200 px-4 py-3 outline-none focus:border-[var(--finder-primary)] focus:ring-2 focus:ring-[var(--finder-primary)]/20 dark:border-zinc-700 dark:bg-zinc-900"><button type="submit" wire:loading.attr="disabled" class="rounded-xl px-5 py-3 text-sm font-semibold text-white disabled:opacity-60" style="background:var(--finder-primary)">Subscrever</button></form>@error('newsletterEmail')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror</div></section>
                    @break
