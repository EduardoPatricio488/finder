@php
    ...
    $targetRoute = match($routeName) {
        'dashboard' => 'site.manage.dashboard',
        'settings' => 'site.manage.settings',
        'definitions' => 'site.manage.definitions',
        'media' => 'site.manage.media',
        'menus' => 'site.manage.menus',
        'categories' => 'site.manage.categories',
        'payments' => 'site.manage.payments',
        'promotions' => 'site.manage.promotions',
        'stock' => 'site.manage.stock',
        'submissions' => 'site.manage.submissions',
        'upgrade' => 'site.manage.upgrade',
        'sales' => 'sales',
        'orders' => 'orders',
        'users' => 'users',
        'products' => 'products',
        default => 'site.manage.dashboard',
    };
@endphp
...
<div class="w-full max-w-sm">
    ...
    <select id="workspace-site-selector" onchange="if (this.value) window.location.href = this.value" class="w-full rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm font-semibold text-zinc-900 shadow-sm outline-none transition hover:border-indigo-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white">
        @foreach($sites as $siteOption)
            <option value="{{ route('site.manage.switch', ['site' => $siteOption, 'redirect' => $targetRoute]) }}" @selected((int) $siteOption->id === (int) $currentSite->id)>{{ $siteOption->name }}</option>
        @endforeach
    </select>
