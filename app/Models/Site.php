...
#[Fillable(['name','slug','owner_id','plan_id','tagline','description','category_label','accent','logo','favicon','primary_color','secondary_color','status','type','subdomain','custom_domain','homepage','social_links','theme','settings','seo','contact_email','phone','address','home_route','is_published','published_at','sort_order'])]
class Site extends Model
{
    use HasFactory;

    protected $attributes = ['category_label'=>'Site','accent'=>'indigo','primary_color'=>'#635bff','secondary_color'=>'#111827','status'=>'draft','type'=>'business','is_published'=>false,'sort_order'=>0];

    protected function casts(): array
    {
        return ['is_published'=>'boolean','homepage'=>'array','social_links'=>'array','theme'=>'array','settings'=>'array','seo'=>'array','published_at'=>'datetime'];
    }
    ...
    #[Scope]
    protected function published(Builder $query): Builder { return $query->where('is_published', true)->where('status', 'published'); }

    public function isProtected(): bool { return $this->slug === 'casa-co' || $this->home_route === 'sales'; }
    public function isVisibleTo(?User $user): bool { return ($this->is_published && $this->status === 'published') || $user?->isAdministrator() === true; }
    public function isManageableBy(?User $user): bool { return $user !== null && ($user->isAdministrator() || $this->owner_id === $user->id || $this->members()->whereKey($user->id)->exists()); }
    public function statusLabel(): string { return match ($this->status) {'published','online'=>'Publicado','draft'=>'Rascunho',default=>'Despublicado'}; }
