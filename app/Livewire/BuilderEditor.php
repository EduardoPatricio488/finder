...
class BuilderEditor extends Component
{
    ...
    public function createPage(): void
    {
        ...
        $page = $this->site->pages()->create([
            'name' => 'Nova página', 'slug' => $slug, 'status' => 'draft', 'is_homepage' => false,
            'sort_order' => $this->site->pages()->max('sort_order') + 1,
        ]);
        $page->sections()->create([
            'type' => 'text', 'label' => 'Conteúdo',
            'content' => ['title' => 'Nova página', 'body' => 'Começa a editar o conteúdo desta página.'],
            'settings' => ['background' => 'transparent', 'padding' => 'lg'], 'sort_order' => 0,
        ]);
        $this->site->load('pages.sections');
        $this->selectPage($page->id);
    }
    ...
    public function duplicatePage(): void
    {
        ...
        $page = $this->site->pages()->create([
            ...
            'seo' => $source->seo, 'sort_order' => $source->sort_order + 1,
        ]);
        foreach ($source->sections as $section) {
            $page->sections()->create([
                'type' => $section->type, 'label' => $section->label, 'content' => $section->content,
                'settings' => $section->settings, 'sort_order' => $section->sort_order, 'is_visible' => $section->is_visible,
            ]);
        }
        $this->site->load('pages.sections');
        $this->selectPage($page->id);
    ...
    {
        $this->sections=$snapshot['sections']; $this->pageName=$snapshot['pageName']; $this->pageSlug=$snapshot['pageSlug']; $this->pageStatus=$snapshot['pageStatus']; $this->theme=$snapshot['theme']; $this->dirty=true;
    }
