...
final class WebsitePublishingService
{
    ...
    public function validate(Site $site): array
    {
        ...
        if (trim((string) $site->name) === '') $errors[] = 'O website precisa de um nome.';
        if ($pages->isEmpty()) $errors[] = 'Cria pelo menos uma página.';
        if (! $pages->contains(fn ($page) => $page->is_homepage)) $errors[] = 'Define uma homepage.';

        foreach ($pages as $page) {
            if (trim((string) $page->name) === '' || trim((string) $page->slug) === '') $errors[] = 'Existe uma página sem nome ou slug.';
            if ($pages->where('slug', $page->slug)->count() > 1) $errors[] = 'O slug "'.$page->slug.'" está duplicado.';
            if ($page->sections->isEmpty()) $errors[] = 'A página "'.$page->name.'" está vazia.';

            $seo = is_array($page->seo) ? $page->seo : [];
            if (blank($seo['title'] ?? null)) $warnings[] = 'A página "'.$page->name.'" não tem título SEO.';
            if (blank($seo['description'] ?? null)) $warnings[] = 'A página "'.$page->name.'" não tem meta descrição.';
        ...
        if (blank($site->favicon)) $warnings[] = 'Adiciona um favicon para uma apresentação mais profissional.';
        if (blank($site->seo['og_image'] ?? null)) $warnings[] = 'Adiciona uma imagem social para melhorar as partilhas.';
        if (blank($site->description) && blank($site->tagline)) $warnings[] = 'Adiciona uma descrição do website para melhorar o SEO e a partilha social.';
    ...
    public function publish(Site $site, ?int $actorId = null): void
    {
        ...
        if (! $result['ok']) abort(422, implode(' ', $result['errors']));
        ...
        $site->forceFill(['is_published' => true, 'status' => 'published', 'published_at' => now()])->save();
    }

    public function unpublish(Site $site): void
    {
        $site->forceFill(['is_published' => false, 'status' => 'draft', 'published_at' => null])->save();
    }

    private function containsUnsafeUrl(array $value): bool
    {
        foreach ($value as $item) {
            if (is_array($item) && $this->containsUnsafeUrl($item)) return true;
            if (is_string($item) && preg_match('/(?:^|\s)(?:javascript|data|vbscript):/i', trim($item))) return true;
        }
        return false;
    }
}
